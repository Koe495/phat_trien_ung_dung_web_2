document.addEventListener('DOMContentLoaded', () => {
            ...
            const clickableSelector = 'a, button, [role="button"], input, .social-btn';
            ...
            // Tự động đo độ rộng thực tế của text nhãn Label (nhân tỷ lệ scale 0.75) 
            // để gán kích thước chính xác cho khoảng trống notch-outline-notch
            function adjustNotchWidth(inputId, labelId, notchId) {
                ...
                function setWidth() {
                    if (input.value !== "" || document.activeElement === input) {
                        // Phép nhân 0.75 để bù trừ độ hẹp của font chữ khi scale(0.75)
                        const width = label.offsetWidth * 0.75;
                        notch.style.width = `${width + 8}px`; // Thêm 8px cho không gian padding lề chữ
                    } else {
                ...
                // Chạy kiểm tra kích thước ban đầu (hữu ích cho autofill hệ thống)
